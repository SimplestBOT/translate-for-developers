...
4. トレイアイコン（T）→ 右クリックメニュー：ホットキー変更／サービス切り替え／言語選択／終了
...
> トレイメニューからのホットキー変更・サービス切り替えはこのファイルに自動保存されます。手動編集は不要です。
...
- **C# ホスト**（`src/csharp/TranslatorHost`、net48 + WinForms + WebView2）：唯一のホスト——
  トレイ、グローバルホットキー、選択キャプチャ（Ctrl+C 注入）、ウィンドウライフサイクル、WebView2、DWM 角丸、ネイティブドラッグ。
...
- **UI ページ**（`src/webui/`、React 19 + TypeScript）：settings/result/capture/config の4ページ、ページ単位のシングルファイルビルド；
